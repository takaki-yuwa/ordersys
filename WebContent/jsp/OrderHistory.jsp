...
<body>
	...
	<main class="history-main">
		...
		<c:if test="${not hasFlag1}">
		    <div class="popup-content" id="popup-content">
		        ...
		        <form action="${Action.ACCOUNTING}" method="post">
		            <button class="popup-proceed" id="confirm-button">
		                <input type="hidden" name="${Param.TABLE_ID}" value="<c:out value='${sessionScope.tableNumber}' />">
		                <input type="hidden" name="${Param.TOTAL}" value="<c:out value='${iTotalPrice}' />">
		                は い
		            </button>
		        </form>
		    </div>
	...
	<footer class="footer-buttons">
		...
		<div class="footer-wrapper">
			<!--ボタン-->
			<!--会計確認ポップアップ表示-->
			<c:if test="${not empty orderHistoryInfo}">
